...
const handleFetchError = (error: FetchError) => {
  ...
  if (error instanceof TypeError && error.cause?.code === 'ECONNREFUSED') {
    console.error(
      'RemoteMfe is not available at address 0:',
      error.cause.errors[0]?.address,
      error.cause.errors[0]?.port
    );

    if (error.cause.errors[1]) {
      console.error(
        'RemoteMfe is not available at address 1:',
        error.cause.errors[1].address,
        error.cause.errors[1].port
      );
    }
  }
};
      ...
      await fetch(_remoteUrl, {
        ...
        .then(async (response) => {
          if (response.ok) {
            ...
            console.error(
              'Failed to connect to RemoteMfe with status:',
              response.status,
              response.statusText
            );
          }
